...
def objective(trial):
    ...
    rf_estimator = trial.suggest_int('rf_estimator', 400, 800, log=True)
    ...
    auc = cross_validate(classifier_obj, train.drop('defects', axis=1),train.defects, cv=5, scoring='roc_auc',fit_params=params)
    return np.mean(auc["test_score"])
...
auc = auc = cross_validate(GradientBoostingClassifier(n_estimators=24, learning_rate=0.19988299934404352, max_depth=4, random_state=23), train.drop('defects', axis=1),train.defects, cv=5, n_jobs=-1, scoring='roc_auc')
