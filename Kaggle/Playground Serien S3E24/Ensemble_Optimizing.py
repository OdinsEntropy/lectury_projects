...
ensemble = VotingClassifier(
    [
     ...
     ('et', make_pipeline(ExtraTreesClassifier(n_estimators=100,min_samples_leaf=100, max_features=1.0,random_state=23,n_jobs=-1))),
...
auc = cross_validate(ensemble, train.drop('smoking', axis=1),train.smoking, cv=5, scoring=['roc_auc'], n_jobs=-1)
...
auc = cross_validate(ensemble, train.drop('smoking', axis=1),train.smoking, cv=5, scoring=['roc_auc'])
result = np.mean(auc["test_roc_auc"])
...
def objective(trial):
    ...
    auc = cross_validate(classifier_obj, train.drop('smoking', axis=1),train.smoking, cv=5, scoring='roc_auc')
...
#%%
ensemble.fit(train.drop('smoking', axis=1),train.smoking)
output_pd = pd.DataFrame({'smoking':ensemble.predict_proba(test)[:, 1]}, index=test.index)
